...
def store(video_path, trajectories, end, args, chunks_data, fps):
    for i, (tube_bbox, start, frame_list) in enumerate(trajectories):
        ...
        name = (video_id + "#" + str(start).zfill(6) +
                "#" + str(end).zfill(6) + ".mp4")
...
def process_video(video_path, detector, args):
    ...
    try:
        for i, frame in enumerate(video):
            ...
            keypoint_scores = predictions.get_field(
                'keypoints').get_field("logits")
            bboxes = predictions.bbox[:, :4]

            # Check if valid person in bbox

            height_criterion = (
                (bboxes[:, 3] - bboxes[:, 1]) > args.mimial_person_size * frame.shape[1]).numpy()
            ...
            intensity_criterion = np.max(np.abs(
                previous_intensity - current_intensity)) > args.intensity_change_threshold
...
if __name__ == "__main__":
    ...
    parser.add_argument("--device_ids", default="0,1",
                        type=str, help="Device to run video on")
    ...
    parser.add_argument("--increase", default=0.05, type=float,
                        help='Increase bbox by this amount')
    parser.add_argument("--min_frames", default=128,
                        type=int, help='Mimimal number of frames')
    ...
    parser.add_argument("--out_folder", default="taichi-256",
                        help="Folder with output videos")
    parser.add_argument("--annotation_folder",
                        default="taichi-annotations", help="Folder for annotations")

    parser.add_argument("--bbox_confidence_th", default=0.9,
                        type=float, help="Maskrcnn confidence for bbox")
    parser.add_argument("--kp_confidence_th", default=2,
                        type=float, help="Maskrcnn confidence for keypoint")
    ...
    parser.add_argument("--minimal_video_size", default=300,
                        type=int, help="Minimal size of the video")

    parser.add_argument("--camera_change_threshold", type=float, default=1)
    parser.add_argument("--intensity_change_threshold",
                        type=float, default=1.5)
    parser.add_argument("--sample_rate", type=int,
                        default=1, help="Sample video rate")
    parser.add_argument("--max_crops", type=int, default=1000,
                        help="Maximal number of crops per video.")
    parser.add_argument("--chunks_metadata", default='taichi-metadata.csv',
